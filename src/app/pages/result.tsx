  ...
  const checkAnswers = () => {
    ...
    activity.questions?.forEach((question: any) => {
      if (question.round_title) {
        ...
        const isCorrect = answerSheet[questionKey].answer == question.is_correct;
  ...
  const renderResults = () => {
    return resultsList.map((question: any, idx: number) => {
      if (question.roundTitle) {
        return (
          <div className='py-8 px-12 w-full text-lg font-bold' key={question.roundTitle}>{question.roundTitle}</div>
        );
      } else {
        return (
          <div 
            key={idx}
            className={`py-4 px-20 flex justify-between ${question.isCorrect ? 'bg-green-50' : 'bg-red-50'}`}
          >
            <p>{question.key}</p>
            <p className={question.isCorrect ? 'text-green-600' : 'text-red-600'}>{question.isCorrect ? "Correct" : "Incorrect"}</p>
          </div>
        )
      }
    })
