...
const QuizPage = () => {
  ...
  const { answerSheet, roundIndex, questionIndex, activityIndex, setAnswerSheet, setQuestionIndex, setRoundIndex, setQuizDone } = useAnswerSheet();
  ...
  const handleSubmitQuestion = (answer: any) => {
    const key = (quizMode == QUIZ_MODE.MULTIPLE ? `R${roundData.order}-` : '') + `Q${question.order}`;
    const newAnswer = {
      answer: answer,
    ...
    if (questionIndex + 1 < questions.length) {
      ...
      setQuestionIndex(0);
      if (quizMode == QUIZ_MODE.MULTIPLE && activity.questions.length > roundIndex + 1) {
  ...
  const sectionLabel = `${activity.activity_name}` + (
    quizMode == QUIZ_MODE.MULTIPLE && !showRoundTitle ? ` / ${roundData.round_title}` : '');

  const titleLabel = showRoundTitle ? roundData.round_title : `Q${question.order}.`;
